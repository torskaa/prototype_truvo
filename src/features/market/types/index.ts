...
export type InstrumentDetailData = { quoteCurrency:string; unit:string; dataSource:string; marketStatus:string; open:number; previousClose:number; dayLow:number; dayHigh:number; historicalLow:number; historicalHigh:number; volumeType:string; performance1d:number; performance1w:number; performance1m:number; performance6m:number; performanceYtd:number; performance1y:number; factors:{ label:string; value:string; unit?:string }[] };
export type FilterRule = { id:string; field:string; operator:string; value:string; join:'AND'|'OR' };
