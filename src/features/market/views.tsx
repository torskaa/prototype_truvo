...
import { ArrowRight, Filter, Lock, Plus, Star, X } from 'lucide-react';
...
const displayMarketSymbol = (instrument: Instrument) =>
 instrument.market === 'Crypto' ? instrument.symbol.split('/')[0] : instrument.symbol;
   ...
   <div className="flex flex-wrap items-end justify-between gap-3">
    ...
    <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-slate-200 bg-white/70 p-2">
     ...
     {durations.map(option => {
      ...
      return <button key={option} type="button" onClick={() => locked ? requestUnlock(requiredTier >= 3 ? 'performanceAnalytics' : 'historicalData') : setDuration(option)} aria-disabled={locked} title={locked ? `Requires Level ${requiredTier}` : `Use ${option} duration`} className={`rounded-md px-2 py-1 text-[9px] font-semibold ${duration === option ? 'bg-violet-600 text-white' : locked ? 'cursor-not-allowed text-slate-300' : 'text-slate-500 hover:bg-violet-50'}`}>{option}{locked && <Lock className="ml-1 inline size-2.5" />}</button>;
     })}
    </div>
   </div>
...
function Screener({ tier, rules, setRules, results, viz, setViz, openInstrument, openIndex, watchlist, toggleWatch, toast, recentTrades }: { tier: Tier; rules: FilterRule[]; setRules: (r: FilterRule[]) => void; results: Instrument[]; viz: Visualization; setViz: (v: Visualization) => void; openInstrument: (i: Instrument) => void; openIndex: (index: MarketIndex) => void; watchlist: string[]; toggleWatch: (symbol: string) => void; toast: (s: string) => void; recentTrades: Array<{ symbol: string; type: 'BUY' | 'SELL'; broker: string; lots: number }> }) {
 ...
 return (
  <>
   <PageHead eyebrow="Discover → Explain → Monitor" title="Instrument Analysis" desc="Review market highlights, Fear & Greed, derivatives activity, and 24-hour changes before selecting an instrument." />
   ...
   <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
    ...
    <div className="seg">{views.map(v => { const tierLocked = snapshot.level.level < visualizationRequiredTier[v]; const featureLocked = v === 'Scatter' && !scatterUnlocked; const unlockFeature = v === 'Heatmap' ? 'heatmap' : v === 'Scatter' ? 'scatter' : v === 'Correlation' ? 'correlation' : v === 'Custom' ? 'customDashboard' : 'advancedScreener'; return <button key={v} onClick={() => { if (tierLocked) { requestUnlock(unlockFeature as Parameters<typeof requestUnlock>[0]); return; } setViz(v); if (v === 'Scatter' && !scatterUnlocked) requestUnlock('advancedScreener'); }} className={viz === v ? 'active' : ''}>{visualizationLabels[v]}{(tierLocked || featureLocked || v === 'Correlation' && !precisionUnlocked) && <Lock className="ml-1 inline size-2.5" />}</button>; })}</div>
   </div>
   ...
   <tbody>
    {sorted.map(i => (
     <tr key={i.symbol} className="group" onClick={() => open(i)}>
      ...
      <td><div className="group/symbol relative inline-flex min-w-16 items-center"><b className="text-slate-900 transition-opacity group-hover/symbol:opacity-0">{displayMarketSymbol(i)}</b>{openBrokerAccess && <div className="absolute left-0 top-1/2 hidden -translate-y-1/2 gap-1 group-hover/symbol:flex"><button type="button" className="secondary px-2 py-1 text-[9px] text-emerald-700" onClick={event => { event.stopPropagation(); openBrokerAccess(); }}>Buy</button><button type="button" className="secondary px-2 py-1 text-[9px] text-rose-700" onClick={event => { event.stopPropagation(); openBrokerAccess(); }}>Sell</button></div>}</div></td>
   ...
   <div className="relative aspect-[5/3] min-h-95 w-full min-w-0 overflow-hidden rounded-xl border border-border bg-slate-50">
    {nodes.map(node => {
     ...
     return <button key={instrument.symbol} onClick={() => open(instrument)} onMouseEnter={() => setHovered(instrument)} onMouseLeave={() => setHovered(null)} className="absolute flex min-h-0 flex-col items-center justify-center overflow-hidden rounded-md border p-1.5 text-center transition-[left,top,width,height,background-color,border-color] duration-500 ease-out hover:z-10 hover:brightness-95" style={{ left: `${node.x0 / 10}%`, top: `${node.y0 / 6}%`, width: `${(node.x1 - node.x0) / 10}%`, height: `${(node.y1 - node.y0) / 6}%`, backgroundColor: color.background, borderColor: color.border }}>
      <b className={`${compact ? 'text-[9px]' : detail ? 'text-sm' : 'text-[10px]'} truncate text-slate-900`}>{displayMarketSymbol(instrument)}</b>
