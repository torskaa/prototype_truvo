...
const taxonomyDemoInstruments: Instrument[] = marketTaxonomy.map((entry, index) => ({
 ...
 name: `${entry.subSector} Demo Basket`,
...
const detailFactors = (instrument: Instrument): InstrumentDetailData['factors'] => {
 if (instrument.market === 'Forex') return [{ label: 'Base currency', value: instrument.symbol.split('/')[0] }, { label: 'Quote currency', value: instrument.symbol.split('/')[1] }, { label: 'Classification', value: instrument.fxCategory ?? 'Major' }, { label: 'Interest-rate differential', value: `${(instrument.change * 0.42).toFixed(2)}%` }, { label: 'Pip value', value: 'Demo · $10 / standard lot' }, { label: 'Central-bank driver', value: 'Demo policy rate spread' }, { label: 'Economic calendar', value: 'CPI · rates · employment' }];
 if (instrument.market === 'Crypto') return [{ label: 'Network', value: instrument.subSector === 'Payments' ? 'XRPL / demo network' : `${instrument.name} network` }, { label: 'Circulating supply', value: `${(instrument.marketCap * 9.4).toFixed(1)}M units` }, { label: 'Total supply', value: 'Protocol-defined' }, { label: 'Fully diluted value', value: `$${(instrument.marketCap * 1.18).toFixed(1)}B` }, { label: 'Funding rate', value: `${(instrument.change * 0.018).toFixed(3)}%` }, { label: 'Open interest', value: `$${(instrument.volume * 0.74).toFixed(1)}B` }, { label: 'Liquidations 24h', value: `$${(instrument.volume * 0.021).toFixed(1)}B` }];
 if (instrument.market === 'Commodity') return [{ label: 'Contract family', value: instrument.subSector ?? 'Metals' }, { label: 'Production / inventory', value: 'Demo inventory series' }, { label: 'Inventory change', value: `${instrument.change >= 0 ? '+' : ''}${(instrument.change * 0.7).toFixed(2)}%` }, { label: 'Spot / futures spread', value: `${(instrument.change * 0.11).toFixed(2)}%` }, { label: 'Seasonality', value: 'Demo monthly pattern' }, { label: 'Macro drivers', value: 'USD · rates · supply' }];
 ...
 return [{ label: 'Index family', value: instrument.name }, { label: 'Constituents', value: 'Demo constituent basket' }, { label: 'Sector breadth', value: `${instrument.change >= 0 ? 'Advancing' : 'Declining'} · demo` }, { label: 'Market breadth', value: `${Math.round(48 + instrument.change * 4)}% positive` }, { label: 'Major contributors', value: 'Technology · Finance' }, { label: 'Major detractors', value: 'Energy · Utilities' }];
};
...
export const instrumentDetailData: Record<string, InstrumentDetailData> = Object.fromEntries(instruments.map(instrument => {
 ...
 return [instrument.symbol, { quoteCurrency: instrument.market === 'Forex' ? instrument.symbol.split('/')[1] : 'USD', unit: instrument.market === 'Forex' ? 'quote' : instrument.market === 'Commodity' ? 'USD / unit' : instrument.market === 'Crypto' ? 'USD / coin' : 'points / share', dataSource: 'Marketsyde Demo Provider', marketStatus: instrument.market === 'Crypto' ? 'Open 24/7' : instrument.market === 'Forex' ? 'Open 24/5' : 'Demo session', open: instrument.price - instrument.change * 0.12, previousClose: instrument.price - instrument.change * 0.2, dayLow: instrument.price - spread, dayHigh: instrument.price + spread, historicalLow: instrument.price * 0.58, historicalHigh: instrument.price * 1.34, volumeType: instrument.market === 'Forex' ? 'Provider tick volume' : instrument.market === 'Crypto' ? '24h trading volume' : instrument.market === 'Commodity' ? 'Contracts / volume' : 'Shares / units traded', performance1d: instrument.change, performance1w: instrument.change * 1.8, performance1m: instrument.return1m, performance6m: instrument.return1m * 3.8, performanceYtd: instrument.return1m * 4.6, performance1y: instrument.return1m * 7.4, factors: detailFactors(instrument) } as InstrumentDetailData];
}));
