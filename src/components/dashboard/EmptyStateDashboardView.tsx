        ...
        <div className="space-y-4 pt-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              ...
              <div
                key={item.name}
                className="p-3 rounded-2xl bg-white border border-slate-200/90 shadow-2xs flex flex-col items-center text-center justify-between space-y-2 hover:shadow-xs transition-shadow"
              >
                <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-slate-900 bg-[#a3e635] px-2 py-0.5 rounded-full">
                  ✔ Verified
                </span>

                <div className={`w-11 h-11 rounded-xl ${item.logoBg} flex items-center justify-center shadow-xs shrink-0`}>
                  {item.renderLogo()}
                </div>

                <div>
                  <div className="font-bold text-xs text-[#0b1c30]">{item.name}</div>
                ...
                <button
                  onClick={() => handleConnectBrokerAction(item.name)}
